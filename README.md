...
```bash
bash -c "$(wget --no-cache -qO- https://raw.githubusercontent.com/programbo/proxmox/main/create.sh)"
```
...
```bash
bash -c "$(wget --no-cache -qO- https://raw.githubusercontent.com/programbo/proxmox/main/create.sh)" -s --app nginx-proxy-manager --cleanup
```
